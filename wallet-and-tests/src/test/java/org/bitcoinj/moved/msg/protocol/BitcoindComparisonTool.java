...
import io.bitcoinsv.bitcoinjsv.core.listeners.PreMessageReceivedEventListener;
...
import io.bitcoinsv.bitcoinjsv.net.NioClient;
        ...
        bitcoind.addPreMessageReceivedEventListener(Threading.SAME_THREAD, new PreMessageReceivedEventListener() {
            ...
            public Message onPreMessageReceived(Peer peer, Message m) {
                if (m instanceof HeadersMessage) {
                    ...
                    for (InventoryItem item : ((GetDataMessage) m).getItems())
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof FullBlockTestGenerator.BlockAndValidity) {
                ...
                if (mostRecentInv == null && !((MemoryPoolState) rule).mempool.isEmpty()) {
                    ...
                    boolean matches = mostRecentInv.getItems().size() == ((MemoryPoolState)rule).mempool.size();
                    for (InventoryItem item : mostRecentInv.getItems())
