...
public class DefaultRiskAnalysisTest {
    ...
    private static final Net NET = Net.MAINNET;
    ...
    public void nonFinal() throws Exception {
        ...
        Transaction tx = new Transaction(NET);
        TransactionInput input = tx.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0));
    ...
    public void selfCreatedAreNotRisky() {
        Transaction tx = new Transaction(NET);
        tx.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0)).setSequenceNumber(1);
        tx.addOutput(COIN, key1);
        tx.setLockTime(TIMESTAMP + 86400);
    ...
    public void nonFinalDependency() {
        ...
        tx1.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0)).setSequenceNumber(1);
    ...
    public void nonStandardDust() {
        ...
        standardTx.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0));
        ...
        dustTx.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0));
        ...
        edgeCaseTx.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0));
    ...
    public void nonShortestPossiblePushData() {
        ...
        tx.addInput(new TransactionInput(NET, null, nonStandardScript));
        ...
        tx.clearInputs();
        assertEquals(DefaultRiskAnalysis.RuleViolation.NONE, DefaultRiskAnalysis.isStandard(tx));
        tx.addOutput(new TransactionOutput(NET, null, COIN, nonStandardScript));
        assertEquals(DefaultRiskAnalysis.RuleViolation.SHORTEST_POSSIBLE_PUSHDATA, DefaultRiskAnalysis.isStandard(tx));
    }
    ...
    public void canonicalSignature() {
        ...
        assertEquals(RuleViolation.NONE,
                DefaultRiskAnalysis.isInputStandard(new TransactionInput(NET, null, scriptOk.getProgram())));
        ...
        assertEquals(RuleViolation.SIGNATURE_CANONICAL_ENCODING,
                DefaultRiskAnalysis.isInputStandard(new TransactionInput(NET, null, scriptUncanonicalEncoding
                        .getProgram())));
    }
    ...
    public void canonicalSignatureLowS() {
        ...
        Script scriptHighS = ScriptBuilder
                .createInputScript(new TransactionSignature(sig.r, ECDSA.CURVE.getN().subtract(sig.s)));
        assertEquals(RuleViolation.SIGNATURE_CANONICAL_ENCODING,
                DefaultRiskAnalysis.isInputStandard(new TransactionInput(NET, null, scriptHighS.getProgram())));

        // This is a real transaction. Its signatures S component is "low".
        Transaction tx1 = new Transaction(NET, Utils.HEX.decode(
        ...
        Transaction tx2 = new Transaction(NET, Utils.HEX.decode(
    ...
    public void standardOutputs() throws Exception {
        Transaction tx = new Transaction(NET);
        tx.addInput(Genesis_legacy.getFor(NET).getTransactions().get(0).getOutput(0));
