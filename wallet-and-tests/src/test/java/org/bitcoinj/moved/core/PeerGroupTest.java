...
import io.bitcoinsv.bitcoinjsv.net.discovery.PeerDiscovery;
...
public class PeerGroupTest extends TestWithPeerGroup {
    ...
    public void receiveTxBroadcast() throws Exception {
        ...
        Assert.assertEquals(2, peerGroup.numConnectedPeers());
        ...
        Coin value = COIN;
        Transaction t1 = FakeTxBuilder.createFakeTx(TestWithNetworkConnections.NET, value, address);
        ...
        inbound(p2, new NotFoundMessage(TestWithNetworkConnections.NET, getdata.getItems()));
        pingAndWait(p2);
    ...
    public void receiveTxBroadcastOnAddedWallet() throws Exception {
        ...
        Wallet wallet2 = new Wallet(TestWithNetworkConnections.PARAMS);
        ECKey key2 = wallet2.freshReceiveKey();
        Address address2 = key2.toAddress(TestWithNetworkConnections.PARAMS);
        
        peerGroup.addTxEventListener(wallet2);
        SPVBlockChain.addChainEventListener(wallet2);

        Assert.assertEquals(BloomFilter.class, waitForOutbound(p1).getClass());
        Assert.assertEquals(MemoryPoolMessage.class, waitForOutbound(p1).getClass());

        Coin value = COIN;
        Transaction t1 = FakeTxBuilder.createFakeTx(TestWithNetworkConnections.NET, value, address2);
        InventoryMessage inv = new InventoryMessage(TestWithNetworkConnections.NET);
        inv.addTransaction(t1);
        ...
        assertNotNull(getdata);
        inbound(p1, new NotFoundMessage(TestWithNetworkConnections.NET, getdata.getItems()));
        pingAndWait(p1);
    ...
    public void transactionConfidence() throws Exception {
        ...
        Transaction tx = FakeTxBuilder.createFakeTx(TestWithNetworkConnections.NET, valueOf(20, 0), address);
        InventoryMessage inv = new InventoryMessage(TestWithNetworkConnections.NET);
    ...
    public void testWalletCatchupTime() throws Exception {
        ...
        Wallet w2 = new Wallet(TestWithNetworkConnections.PARAMS);
        ECKey key1 = new ECKey();
        key1.setCreationTimeSeconds(now - 86400);  // One day ago.
        w2.importKey(key1);
        peerGroup.addTxEventListener(w2);
        peerGroup.waitForJobQueue();
        Assert.assertEquals(peerGroup.getFastCatchupTimeSecs(), now - 86400 - WEEK);
        ...
        w2.importKey(key2);
        peerGroup.waitForJobQueue();
        Assert.assertEquals(peerGroup.getFastCatchupTimeSecs(), now - WEEK - 100000);
    }
    ...
    public void noPings() throws Exception {
        ...
        VersionMessage versionMessage = new VersionMessage(TestWithNetworkConnections.NET, 2);
    ...
    public void downloadPeerSelection() throws Exception {
        peerGroup.start();
        VersionMessage versionMessage2 = new VersionMessage(TestWithNetworkConnections.NET, 2);
        ...
        VersionMessage versionMessage3 = new VersionMessage(TestWithNetworkConnections.NET, 3);
        ...
        Assert.assertEquals(2, peerGroup.getMostCommonChainHeight());
        Assert.assertEquals(a, peerGroup.getDownloadPeer());
        connectPeer(2, versionMessage2);
        Assert.assertEquals(2, peerGroup.getMostCommonChainHeight());
        Assert.assertEquals(a, peerGroup.getDownloadPeer());  // No change.
        Peer c = connectPeer(3, versionMessage3).peer;
        Assert.assertEquals(2, peerGroup.getMostCommonChainHeight());
        Assert.assertEquals(a, peerGroup.getDownloadPeer());  // No change yet.
        connectPeer(4, versionMessage3);
        Assert.assertEquals(3, peerGroup.getMostCommonChainHeight());
        Assert.assertEquals(a, peerGroup.getDownloadPeer());  // Still no change.
    ...
    public void testBloomOnP2Pubkey() throws Exception {
        ...
        Transaction tx = FakeTxBuilder.createFakeTx(TestWithNetworkConnections.NET, COIN, key);
        Transaction tx2 = new Transaction(TestWithNetworkConnections.NET);
        ...
        inbound(p1, new NotFoundMessage(TestWithNetworkConnections.NET, ImmutableList.of(inv)));
        assertNull(outbound(p1));
        assertNull(outbound(p2));
        peerGroup.waitForJobQueue();
    ...
    public void testBloomResendOnNewKey() throws Exception {
        ...
        while ((bf = (BloomFilter) outbound(p1)) != null) {
            Assert.assertEquals(MemoryPoolMessage.class, outbound(p1).getClass());
            f2 = bf;
        }
        assertNotNull(key);
        assertNotNull(f2);
    ...
    public void waitForPeersOfVersion() throws Exception {
        ...
        VersionMessage ver1 = new VersionMessage(TestWithNetworkConnections.NET, 10);
        ver1.clientVersion = baseVer;
        ver1.localServices = VersionMessage.NODE_NETWORK | VersionMessage.NODE_BITCOIN_CASH;
        VersionMessage ver2 = new VersionMessage(TestWithNetworkConnections.NET, 10);
    ...
    public void waitForPeersWithServiceFlags() throws Exception {
        ...
        connectPeer(2, ver2);
        assertFalse(future.isDone());
        Assert.assertEquals(1, peerGroup.findPeersWithServiceMask(3).size());
    ...
    public void preferLocalPeer() throws IOException {
        ...
        try {
            local = new ServerSocket(TestWithNetworkConnections.PARAMS.getPort(), 100, InetAddresses.forString("127.0.0.1"));
        }
        ...
        try {
            ...
            Assert.assertEquals(1, peerGroup.getMaxConnections());
            Assert.assertEquals(PeerAddress.localhost(TestWithNetworkConnections.PARAMS), peerGroup.getPendingPeers().get(0).getAddress());
        } finally {
            local.close();
        }
    }
    ...
    public void autoRescanOnKeyExhaustion() throws Exception {
        ...
        for (ECKey key1 : keys) {
            Address addr = key1.toAddress(TestWithNetworkConnections.PARAMS);
            Block next = FakeTxBuilder.makeSolvedTestBlock(prev, FakeTxBuilder.createFakeTx(TestWithNetworkConnections.NET, Coin.FIFTY_COINS, addr));
        ...
        BloomFilter filter = new BloomFilter(TestWithNetworkConnections.NET, p1.lastReceivedFilter.bitcoinSerialize());
        ...
        Assert.assertEquals(exhaustionPoint.getPrevBlockHash(), SPVBlockChain.getChainHead().getHeader().getHash());
        ...
        Assert.assertEquals(blocks.get(blocks.size() - 1).getHash(), SPVBlockChain.getChainHead().getHeader().getHash());
    }
