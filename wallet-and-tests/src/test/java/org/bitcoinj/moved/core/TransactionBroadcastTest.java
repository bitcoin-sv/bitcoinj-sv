...
import io.bitcoinsv.bitcoinjsv.msg.p2p.InventoryMessage;
import io.bitcoinsv.bitcoinjsv.msg.p2p.RejectMessage;
...
public class TransactionBroadcastTest extends TestWithPeerGroup {
    ...
    public void rejectHandling() throws Exception {
        ...
        ListenableFuture<Transaction> future = broadcast.broadcast();
        // 0 and 3 are randomly selected to receive the broadcast.
        Assert.assertEquals(tx, outbound(channels[1]));
        Assert.assertEquals(tx, outbound(channels[2]));
        Assert.assertEquals(tx, outbound(channels[4]));
        RejectMessage reject = new RejectMessage(TestWithNetworkConnections.NET, RejectMessage.RejectCode.DUST, tx.getHash(), "tx", "dust");
    ...
    public void retryFailedBroadcast() throws Exception {
        ...
        Address dest = new ECKey().toAddress(TestWithNetworkConnections.PARAMS);
    ...
    public void peerGroupWalletIntegration() throws Exception {
        ...
        VersionMessage ver = new VersionMessage(TestWithNetworkConnections.NET, 2);
        ...
        Assert.assertEquals(t3.getHash(), outbound(p1).getHash());
    }
}
