...
import io.bitcoinj.exception.HDDerivationException;
...
public final class HDKeyDerivation {
    ...
    public static RawKeyBytes deriveChildKeyBytesFromPublic(DeterministicKey parent, ChildNumber childNumber, PublicDeriveMode mode) throws HDDerivationException {
        ...
        switch (mode) {
            case NORMAL:
                Ki = ECKeyLite.publicPointFromPrivate(ilInt).add(parent.getPubKeyPoint());
                break;
            case WITH_INVERSION:
                ...
                Ki = ECKeyLite.publicPointFromPrivate(ilInt.add(RAND_INT).mod(N));
                BigInteger additiveInverse = RAND_INT.negate().mod(N);
                Ki = Ki.add(ECKeyLite.publicPointFromPrivate(additiveInverse));
