 ...
 */
public class DeterministicKey extends ECKeyLite {

    /** Sorts deterministic keys in the order of their child number. That's <i>usually</i> the order used to derive them. */
    public static final Comparator<ECKeyLite> CHILDNUM_ORDER = new Comparator<ECKeyLite>() {
        @Override
    ...
    public DeterministicKey(ImmutableList<ChildNumber> childNumberPath,
                            ...
                            BigInteger priv,
                            @Nullable DeterministicKey parent) {
        super(priv, compressPoint(ECKeyLite.publicPointFromPrivate(priv)));
        checkArgument(chainCode.length == 32);
        this.parent = parent;
        this.childNumberPath = checkNotNull(childNumberPath);
        this.chainCode = Arrays.copyOf(chainCode, chainCode.length);
     ...
     * in which case the time is stored alongside the key as per normal, see {@link io.bitcoinj.core.ECKeyLite#getCreationTimeSeconds()}.
     */
    @Override
