...
import io.bitcoinsv.bitcoinjsv.core.VersionedChecksummedBytes;
import io.bitcoinsv.bitcoinjsv.ecc.ECDSA;
import io.bitcoinsv.bitcoinjsv.exception.AddressFormatException;
import io.bitcoinsv.bitcoinjsv.exception.BadPassphraseException;
...
public class BIP38PrivateKey extends VersionedChecksummedBytes {
    ...
    public ECKeyLite decrypt(String passphrase) throws BadPassphraseException {
        String normalizedPassphrase = Normalizer.normalize(passphrase, Normalizer.Form.NFC);
        ECKeyLite key = ecMultiply ? decryptEC(normalizedPassphrase) : decryptNoEC(normalizedPassphrase);
    ...
    private ECKeyLite decryptNoEC(String normalizedPassphrase) {
        try {
            ...
            return ECKeyLite.fromPrivate(decrypted, compressed);
        } catch (GeneralSecurityException x) {
            throw new RuntimeException(x);
        }
    }

    private ECKeyLite decryptEC(String normalizedPassphrase) {
        try {
            ...
            ECKeyLite k = ECKeyLite.fromPrivate(passFactor, true);
            ...
            return ECKeyLite.fromPrivate(priv, compressed);
        } catch (GeneralSecurityException x) {
            throw new RuntimeException(x);
        }
    }
