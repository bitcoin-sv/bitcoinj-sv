...
import io.bitcoinsv.bitcoinjsv.bitcoin.bean.validator.TxBeanValidator;
...
public class ScriptTest {
    ...
    public void testScriptPubKey() throws Exception {
        ...
        AddressLite toAddr = new AddressLite(PARAMS, pubkey.getPubKeyHash());
    ...
    public void testMultiSig() throws Exception {
        List<ECKeyLite> keys = Lists.newArrayList(new ECKeyLite(), new ECKeyLite(), new ECKeyLite());
    ...
    public void dataDrivenValidTransactions() throws Exception {
        ...
        for (JsonNode test : json) {
            ...
            try {
                ...
                transaction = new TxBean(Utils.HEX.decode(test.get(1).asText().toLowerCase()));
    ...
    public void dataDrivenInvalidTransactions() throws Exception {
        ...
        for (JsonNode test : json) {
            if (test.isArray() && test.size() == 1 && test.get(0).isTextual())
                continue; // This is a comment.
            Map<TxOutPoint, Script> scriptPubKeys = parseScriptPubKeys(test.get(0));
            Tx transaction = new TxBean(Utils.HEX.decode(test.get(1).asText().toLowerCase()));
            Set<ScriptVerifyFlag> verifyFlags = parseVerifyFlags(test.get(2).asText());
