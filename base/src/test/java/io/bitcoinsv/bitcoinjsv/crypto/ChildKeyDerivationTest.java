...
public class ChildKeyDerivationTest {
    ...
    public void testChildKeyDerivation() throws Exception {
        ...
        for(int i = 0; i < 1; i++) {
            byte[] priv  = Utils.HEX.decode(ckdTestVectors[3 * i]);
            byte[] pub   = Utils.HEX.decode(ckdTestVectors[3 * i + 1]);
            byte[] chain = Utils.HEX.decode(ckdTestVectors[3 * i + 2]); // chain code
...
//            ExtendedHierarchicKey ekpub_1_IN_4bil = HDKeyDerivation.deriveChildKey(ekpub_1_IN, 4294967295L);
    ...
    public void encryptedDerivation() throws Exception {
        ...
        try {
            ...
            fail();
        } catch (ECKeyLite.KeyIsEncryptedException e) {
            // Ignored.
        }
        ECDSASignature signature = derivedKey2.sign(hash, aesKey);
        Assertions.assertTrue(derivedKey2.verify(hash, signature));
    }
    ...
    public void pubOnlyDerivation() throws Exception {
        ...
        Assertions.assertEquals(key3.getPubKeyPoint(), pubkey3.getPubKeyPoint());
    }
    ...
    public void serializeToTextAndBytes() {
...

        {
            ...
            Assertions.assertEquals(DeterministicKey.deserializeB58(null, pub58, params).getPubKeyPoint(), key1.getPubKeyPoint());
            Assertions.assertEquals(DeterministicKey.deserializeB58(pub58, params).getPubKeyPoint(), key1.getPubKeyPoint());
            ...
            Assertions.assertEquals(DeterministicKey.deserialize(params, pub, null).getPubKeyPoint(), key1.getPubKeyPoint());
            Assertions.assertEquals(DeterministicKey.deserialize(params, pub).getPubKeyPoint(), key1.getPubKeyPoint());
        }
        {
            ...
            Assertions.assertEquals(DeterministicKey.deserializeB58(key1, pub58, params).getPubKeyPoint(), key2.getPubKeyPoint());
            assertEquals(DeterministicKey.deserialize(params, priv, key1), key2);
            Assertions.assertEquals(DeterministicKey.deserialize(params, pub, key1).getPubKeyPoint(), key2.getPubKeyPoint());
        }
    }
    ...
    public void reserialization() {
        ...
        assertEquals(key.serializePubB58(MainNetParams.get()), encoded, "Reserialized parentless private HD key is wrong");
        ...
        assertEquals(key.getParentFingerprint(), 0xbef5a2f9, "Parent fingerprint of deserialized parentless public HD key is wrong");
        ...
        assertEquals( key.serializePrivB58(MainNetParams.get()), encoded, "Reserialized parentless private HD key is wrong");
        ...
        assertEquals(key.getParentFingerprint(), 0xbef5a2f9, "Parent fingerprint of deserialized parentless private HD key is wrong");

        // These encodings are of the the root key of that hierarchy
        assertEquals(DeterministicKey.deserializeB58("xpub661MyMwAqRbcFW31YEwpkMuc5THy2PSt5bDMsktWQcFF8syAmRUapSCGu8ED9W6oDMSgv6Zz8idoc4a6mr8BDzTJY47LJhkJ8UB7WEGuduB", MainNetParams.get()).getParentFingerprint(),
                          0, "Parent fingerprint of root node public HD key should be zero");
        assertEquals(DeterministicKey.deserializeB58("xprv9s21ZrQH143K31xYSDQpPDxsXRTUcvj2iNHm5NUtrGiGG5e2DtALGdso3pGz6ssrdK4PFmM8NSpSBHNqPqm55Qn3LqFtT2emdEXVYsCzC2U", MainNetParams.get()).getParentFingerprint(),
                          0, "Parent fingerprint of root node private HD key should be zero");
