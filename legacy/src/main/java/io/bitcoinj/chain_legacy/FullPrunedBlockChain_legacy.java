 ...
 * Core does.</p>
 */
public class FullPrunedBlockChain_legacy extends AbstractBlockChain_legacy {
    private static final Logger log = LoggerFactory.getLogger(FullPrunedBlockChain_legacy.class);
    ...
    @Override
    protected StoredBlock_legacy addToBlockStore(StoredBlock_legacy storedPrev, Block header, TransactionOutputChanges txOutChanges)
            throws BlockStoreException, VerificationException {
        StoredBlock_legacy newBlock = storedPrev.build(header);
        blockStore.put(newBlock, new StoredUndoableBlock_legacy(newBlock.getHeader().getHash(), txOutChanges));
        return newBlock;
    }

    @Override
    protected StoredBlock_legacy addToBlockStore(StoredBlock_legacy storedPrev, Block block)
            throws BlockStoreException, VerificationException {
        StoredBlock_legacy newBlock = storedPrev.build(block);
        blockStore.put(newBlock, new StoredUndoableBlock_legacy(newBlock.getHeader().getHash(), block.getParsedTransactions()));
        return newBlock;
    }
    ...
    private static class Verifier implements Callable<VerificationException> {
        ...
        public Verifier(final Transaction tx, final List<Script> prevOutScripts, final Set<ScriptVerifyFlag> verifyFlags) {
            this.tx = tx;
        ...
        public VerificationException call() throws Exception {
            try {
                ...
                for (int index = 0; index < tx.getInputs().size(); index++) {
                    ...
                    ScriptUtils_legacy.correctlySpends(tx.getInputs().get(index).getScriptSig(), tx, index, prevOutIt.next(), value, verifyFlags);
                }
        ...
        try {
            if (!params.isCheckpoint(height)) {
                ...
                for (Transaction tx : block.getParsedTransactions()) {
                    Integer countaboveBip65 = getVersionTally().getCountAtOrAbove(BitcoinJ.BLOCK_VERSION_BIP65);
                    final Set<ScriptVerifyFlag> verifyFlags = Verification.getTransactionVerificationFlags(params, block, countaboveBip65, height);
                    ...
                    if (verifyFlags.contains(ScriptVerifyFlag.P2SH)) // We already check non-BIP16 sigops in Block.verifyTransactions(true)
     ...
     */
    protected synchronized TransactionOutputChanges connectTransactions(StoredBlock_legacy newBlock)
        ...
        blockStore.beginDatabaseBatchWrite();
        StoredUndoableBlock_legacy block = blockStore.getUndoBlock(newBlock.getHeader().getHash());
        if (block == null) {
        ...
        try {
            ...
            if (transactions != null) {
                ...
                for (final Transaction tx : transactions) {
                    int countAboveBip34 = getVersionTally().getCountAtOrAbove(BitcoinJ.BLOCK_VERSION_BIP34);
                    final Set<ScriptVerifyFlag> verifyFlags =
                            Verification.getTransactionVerificationFlags(params, newBlock.getHeader(), countAboveBip34, Integer.SIZE);
                    boolean isCoinBase = tx.isCoinBase();
                    Coin valueIn = Coin.ZERO;
                    Coin valueOut = Coin.ZERO;
                    final List<Script> prevOutScripts = new LinkedList<Script>();
     ...
     */
    @Override
    protected void disconnectTransactions(StoredBlock_legacy oldBlock) throws PrunedException, BlockStoreException {
        checkState(lock.isHeldByCurrentThread());
        blockStore.beginDatabaseBatchWrite();
        try {
            StoredUndoableBlock_legacy undoBlock = blockStore.getUndoBlock(oldBlock.getHeader().getHash());
