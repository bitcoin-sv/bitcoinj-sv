...
package io.bitcoinj.store_legacy;

import io.bitcoinj.blockstore.ChainFileLockedException;
import io.bitcoinj.blockstore.WindowsMMapHack;
 ...
 */
public class SPVBlockStore_legacy implements BlockStore_legacy {
    private static final Logger log = LoggerFactory.getLogger(SPVBlockStore_legacy.class);
    ...
    protected LinkedHashMap<Sha256Hash, StoredBlock_legacy> blockCache = new LinkedHashMap<Sha256Hash, StoredBlock_legacy>() {
        @Override
        protected boolean removeEldestEntry(Map.Entry<Sha256Hash, StoredBlock_legacy> entry) {
     ...
     * will block on disk.
     */
    public SPVBlockStore_legacy(NetworkParameters params, File file) throws BlockStoreException {
        checkNotNull(file);
    ...
    public StoredBlock_legacy get(Sha256Hash hash) throws BlockStoreException {
        ...
        lock.lock();
        try {
            StoredBlock_legacy cacheHit = blockCache.get(hash);
            if (cacheHit != null)
                return cacheHit;
            ...
            do {
                ...
                if (Arrays.equals(scratch, targetHashBytes)) {
                    // Found the target.
                    StoredBlock_legacy storedBlock = StoredBlock_legacy.deserializeCompact(params, buffer);
    ...
    protected StoredBlock_legacy lastChainHead = null;
    ...
    public StoredBlock_legacy getChainHead() throws BlockStoreException {
        ...
        try {
            if (lastChainHead == null) {
                ...
                StoredBlock_legacy block = get(hash);
                if (block == null)
    ...
    protected static final int RECORD_SIZE = 32 /* hash */ + StoredBlock_legacy.COMPACT_SERIALIZED_SIZE;
