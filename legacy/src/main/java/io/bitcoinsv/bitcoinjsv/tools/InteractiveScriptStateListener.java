...
import io.bitcoinsv.bitcoinjsv.script.interpreter.*;
...
public class InteractiveScriptStateListener extends ScriptStateListener {
    ...
    public static void main(String[] args) {
        ...
        byte[] tx1Bytes = Utils.HEX.decode(rawTx1);
        Transaction tx1 = new Transaction(net, tx1Bytes);

        byte[] tx2Bytes = Utils.HEX.decode(rawtx2);
...
//        System.out.println();
        ...
        if (reverseStack.isEmpty()) {
          System.out.println("empty");
            ...
            for (StackItem item : reverseStack) {
                ...
                System.out.println(String.format("index[%s] len[%s] [%s]", index++, item.length(), Utils.HEX.encode(item.bytes())));
        ...
        if (!getAltStack().isEmpty()) {
            ...
            for (StackItem item: reverseStack) {
                System.out.println(Utils.HEX.encode(item.bytes()));
            }
            System.out.println();
        }
