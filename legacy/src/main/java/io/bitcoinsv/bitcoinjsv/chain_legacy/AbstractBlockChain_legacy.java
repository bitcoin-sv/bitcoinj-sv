...
import io.bitcoinsv.bitcoinjsv.pow_legacy.factory.RuleCheckerFactory_legacy;
...
import io.bitcoinsv.bitcoinjsv.store_legacy.H2FullPrunedBlockStore;
import io.bitcoinsv.bitcoinjsv.store_legacy.SPVBlockStore_legacy;
...
import io.bitcoinsv.bitcoinjsv.utils.VersionTally_legacy;
import org.slf4j.*;
 ...
 *
 * <p>An AbstractBlockChain implementation must be connected to a {@link BlockStore_legacy} implementation. The chain object
 ...
 * the lightweight SPV mode, a {@link SPVBlockStore_legacy} is the right choice.</p>
 *
 ...
 */
public abstract class AbstractBlockChain_legacy {
    private static final Logger log = LoggerFactory.getLogger(AbstractBlockChain_legacy.class);
    protected final ReentrantLock lock = Threading.lock("blockchain");

    /** Keeps a map of block hashes to StoredBlocks. */
    private final BlockStore_legacy blockStore;
     ...
     */
    protected StoredBlock_legacy chainHead;
    ...
    protected final AbstractRuleCheckerFactory_legacy ruleCheckerFactory;
    ...
    private final VersionTally_legacy versionTally;
    ...
    public AbstractBlockChain_legacy(NetworkParameters params, List<? extends ChainEventListener> wallets,
        ...
        this.params = params;
        this.ruleCheckerFactory = RuleCheckerFactory_legacy.create(this.params);
    ...
    /**
     * Returns the {@link BlockStore_legacy} the chain was constructed with. You can use this to iterate over the chain.
     */
    public BlockStore_legacy getBlockStore() {
        return blockStore;
    }
     ...
     * @param storedPrev The {@link StoredBlock_legacy} which immediately precedes block.
     * @param block The {@link Block} to add/update.
     * @return the newly created {@link StoredBlock_legacy}
     */
    protected abstract StoredBlock_legacy addToBlockStore(StoredBlock_legacy storedPrev, Block block)
            throws BlockStoreException, VerificationException;
    
    /**
     * Adds/updates the given {@link StoredBlock_legacy} with the block store.
     * This version is used when the transactions have already been verified to properly spend txOutputChanges.
     * @param storedPrev The {@link StoredBlock_legacy} which immediately precedes block.
     * @param header The {@link StoredBlock_legacy} to add/update.
     ...
     * @return the newly created {@link StoredBlock_legacy}
     */
    protected abstract StoredBlock_legacy addToBlockStore(StoredBlock_legacy storedPrev, Block header,
     ...
     */
    protected abstract void doSetChainHead(StoredBlock_legacy chainHead) throws BlockStoreException;
     ...
     */
    protected abstract StoredBlock_legacy getStoredBlockInCurrentScope(Sha256Hash hash) throws BlockStoreException;
     ...
     * If an error is encountered in a transaction, no changes should be made to the underlying BlockStore.
     * Only called if(shouldVerifyTransactions())
     * @throws PrunedException if newBlock does not exist as a {@link StoredUndoableBlock_legacy} in the block store.
     * @throws VerificationException if an attempt was made to spend an already-spent output, or if a transaction incorrectly solved an output script.
     ...
     * @return The full set of all changes made to the set of open transaction outputs.
     */
    protected abstract TransactionOutputChanges connectTransactions(StoredBlock_legacy newBlock) throws VerificationException, BlockStoreException, PrunedException;
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        try {
            ...
            final StoredBlock_legacy storedPrev;
            final int height;
            ...
            try {
                block.verifyHeader(params.getNet());
                ...
                Integer countAboveBip34 = versionTally.getCountAtOrAbove(BitcoinJ.BLOCK_VERSION_BIP34);
                flags = Verification.getBlockVerificationFlags(params, block, countAboveBip34, height);
            ...
            if (storedPrev == null) {
                ...
                AbstractPowRulesChecker_legacy rulesChecker = ruleCheckerFactory.getRuleChecker(storedPrev, block);
    ...
    private void connectBlock(final Block block, StoredBlock_legacy storedPrev, boolean expensiveChecks,
        ...
        StoredBlock_legacy head = getChainHead();
        if (storedPrev.equals(head)) {
            ...
            StoredBlock_legacy newStoredBlock = addToBlockStore(storedPrev,
            ...
            informListenersForNewBlock(block, AbstractBlockChain.NewBlockType.BEST_CHAIN, filteredTxHashList, filteredTxn, newStoredBlock);
        } else {
            // This block connects to somewhere other than the top of the best known chain. We treat these differently.
            //
            // Note that we send the transactions to the wallet FIRST, even if we're about to re-organize this block
            // to become the new best chain head. This simplifies handling of the re-org in the Wallet class.
            StoredBlock_legacy newBlock = storedPrev.build(block);
            boolean haveNewBestChain = newBlock.moreWorkThan(head);
            if (haveNewBestChain) {
                log.info("Block is causing a re-organize");
            } else {
                StoredBlock_legacy splitPoint = findSplit(newBlock, head, blockStore);
            ...
            if (block.getParsedTransactions() != null || filtered) {
                informListenersForNewBlock(block, AbstractBlockChain.NewBlockType.SIDE_CHAIN, filteredTxHashList, filteredTxn, newBlock);
            }
    ...
    private void informListenersForNewBlock(final Block block, final AbstractBlockChain.NewBlockType newBlockType,
                                            @Nullable final List<Sha256Hash> filteredTxHashList,
                                            @Nullable final Map<Sha256Hash, Transaction> filteredTxn,
                                            final StoredBlock_legacy newStoredBlock) throws VerificationException {
    ...
    private static void informListenerForNewTransactions(Block block, AbstractBlockChain.NewBlockType newBlockType,
                                                         @Nullable List<Sha256Hash> filteredTxHashList,
                                                         @Nullable Map<Sha256Hash, Transaction> filteredTxn,
                                                         StoredBlock_legacy newStoredBlock, boolean first,
     ...
     */
    public static long getMedianTimestampOfRecentBlocks(StoredBlock_legacy storedBlock,
                                                        BlockStore_legacy store) throws BlockStoreException {
     ...
     * Only called if(shouldVerifyTransactions())
     * @throws PrunedException if block does not exist as a {@link StoredUndoableBlock_legacy} in the block store.
     * @throws BlockStoreException if the block store had an underlying error or block does not exist in the block store at all.
     */
    protected abstract void disconnectTransactions(StoredBlock_legacy block) throws PrunedException, BlockStoreException;
     ...
     */
    private void handleNewBestChain(StoredBlock_legacy storedPrev, StoredBlock_legacy newChainHead, Block block, boolean expensiveChecks)
            throws BlockStoreException, VerificationException, PrunedException {
        checkState(lock.isHeldByCurrentThread());
        ...
        StoredBlock_legacy head = getChainHead();
        final StoredBlock_legacy splitPoint = findSplit(newChainHead, head, blockStore);
        log.info("Re-organize after split at height {}", splitPoint.getHeight());
        log.info("Old chain head: {}", head.getHeader().getHashAsString());
        log.info("New chain head: {}", newChainHead.getHeader().getHashAsString());
        log.info("Split at block: {}", splitPoint.getHeader().getHashAsString());
        // Then build a list of all blocks in the old part of the chain and the new part.
        final LinkedList<StoredBlock_legacy> oldBlocks = getPartialChain(head, splitPoint, blockStore);
        final LinkedList<ChainInfoReadOnly> oldChainInfo = new LinkedList<>(oldBlocks);
        final LinkedList<StoredBlock_legacy> newBlocks = getPartialChain(newChainHead, splitPoint, blockStore);
        final LinkedList<ChainInfoReadOnly> newChainInfo = new LinkedList<>(newBlocks);
        // Disconnect each transaction in the previous main chain that is no longer in the new main chain
        StoredBlock_legacy storedNewHead = splitPoint;
        if (shouldVerifyTransactions()) {
            for (StoredBlock_legacy oldBlock : oldBlocks) {
                try {
            ...
            StoredBlock_legacy cursor;
            // Walk in ascending chronological order.
            for (Iterator<StoredBlock_legacy> it = newBlocks.descendingIterator(); it.hasNext();) {
                cursor = it.next();
     ...
     */
    private static LinkedList<StoredBlock_legacy> getPartialChain(StoredBlock_legacy higher, StoredBlock_legacy lower, BlockStore_legacy store) throws BlockStoreException {
        checkArgument(higher.getHeight() > lower.getHeight(), "higher and lower are reversed");
        LinkedList<StoredBlock_legacy> results = new LinkedList<StoredBlock_legacy>();
        StoredBlock_legacy cursor = higher;
        while (true) {
            results.add(cursor);
     ...
     */
    private static StoredBlock_legacy findSplit(StoredBlock_legacy newChainHead, StoredBlock_legacy oldChainHead,
                                                BlockStore_legacy store) throws BlockStoreException {
        StoredBlock_legacy currentChainCursor = oldChainHead;
        StoredBlock_legacy newChainCursor = newChainHead;
    ...
    private static void sendTransactionsToListener(StoredBlock_legacy block, AbstractBlockChain.NewBlockType blockType,
                                                   TransactionReceivedInBlockListener listener,
    ...
    protected void setChainHead(StoredBlock_legacy chainHead) throws BlockStoreException {
    ...
    private void tryConnectingOrphans() throws VerificationException, BlockStoreException, PrunedException {
        ...
        do {
            ...
            while (iter.hasNext()) {
                ...
                StoredBlock_legacy prev = getStoredBlockInCurrentScope(orphanBlock.block.getPrevBlockHash());
                if (prev == null) {
     ...
     */
    public StoredBlock_legacy getChainHead() {
        synchronized (chainHeadLock) {
            return chainHead;
        }
    }
     ...
     * {@link StoredBlock_legacy} of the block that reaches that height first. The future completes on a peer thread.
     */
    public ListenableFuture<StoredBlock_legacy> getHeightFuture(final int height) {
        final SettableFuture<StoredBlock_legacy> result = SettableFuture.create();
        addNewBestBlockListener(Threading.SAME_THREAD, new NewBestBlockListener() {
            ...
            public void notifyNewBestBlock(ChainInfoReadOnly block) throws VerificationException {
                if (block.getHeight() >= height) {
                    ...
                    result.set((StoredBlock_legacy) block);
                }
            }
        });
        return result;
    ...
    protected VersionTally_legacy getVersionTally() {
        return versionTally;
    }
}
