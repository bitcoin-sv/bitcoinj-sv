...
package io.bitcoinsv.bitcoinjsv.testing;
...
public class FakeTxBuilder {
    ...
    public static Transaction createFakeTxWithChangeAddress(Net net, Coin value, Address to, Address changeOutput) {
        ...
        TransactionOutput change = new TransactionOutput(net, t, Coin.valueOf(1, 11), changeOutput);
        t.addOutput(change);
    ...
    public static Transaction createFakeTx(Net net, Coin value, ECKey to) {
        ...
        TransactionOutput outputToMe = new TransactionOutput(net, t, value, to);
        t.addOutput(outputToMe);
        TransactionOutput change = new TransactionOutput(net, t, Coin.valueOf(1, 11), new ECKey());
        t.addOutput(change);
        // Make a previous tx simply to send us sufficient coins. This prev tx is not really valid but it doesn't
        // matter for our purposes.
        Transaction prevTx = new Transaction(net);
    ...
    public static Transaction[] createFakeTx(Net net, Coin value,
        ...
        TransactionOutput outputToMe = new TransactionOutput(net, t, value, to);
        t.addOutput(outputToMe);
        TransactionOutput change = new TransactionOutput(net, t, Coin.valueOf(1, 11), new ECKey().toAddress(net.params()));
        t.addOutput(change);
    ...
    public static BlockPair createFakeBlock(BlockStore_legacy blockStore, StoredBlock_legacy previousStoredBlock, long version,
        ...
        try {
            ...
            for (Transaction tx : transactions) {
                TxHelper.getConfidence(tx).setSource(TransactionConfidence.Source.NETWORK);
                b.addTransaction(tx);
            }
            b.solve();
